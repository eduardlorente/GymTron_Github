...
public interface IRoutineRepository : IRepository<Routine>
{
    ...
    Task<Routine?> GetById(int id, CancellationToken cancellationToken = default);
    Task<RoutineProjection?> GetRoutineProjection(int id, CancellationToken cancellationToken = default);
